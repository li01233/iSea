...
class MobileViTBlock(nn.Module):
    def __init__(self, in_channel,out_channel, kernel_size, patch_size, mlp_dim, depth=3, dim=96, dropout=0.):
        super().__init__()
...
def prime_factors(n):
   # 从3开始遍历到sqrt(n)，判断每个数字是否为质数
    for i in range(3, int(n**0.5)+1):
        if n % i == 0:
            return i   
                
    # 如果最后还有余数，则说明该数本身就是一个大于sqrt(n)的质数
    if n > 1:
        return n